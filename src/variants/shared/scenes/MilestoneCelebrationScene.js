...
// top-view hero (heroTop frames 1..4, looped) as large as fits on the LEFT, text on the RIGHT showing
// this run's {walls}/{seconds}. "Tap anywhere to continue" → the normal Game Over screen (score
// carried through). Variant-only.
...
import { startConfetti, startFireworks, fillTokens, makeAnimatedHero } from './celebrationFx.js';
...
export class MilestoneCelebrationScene extends Phaser.Scene {
  ...
  create(data) {
    ...
    const title = this.add.text(tx, si.top + Math.round(H * 0.10), fillTokens(GT.milestoneTitle, tokens), {
    ...
    const tap = this.add.text(tx, H - si.bottom - Math.round(22 * s), GT.milestoneTapContinue, {
